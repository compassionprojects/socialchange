...
describe NotificationMailer, type: :mailer do
  describe "#notify_new_story" do
    ...
    let(:mail) { described_class.with(recipient: user, record: story).notify_new_story }
  ...
  describe "#notify_new_discussion" do
    ...
    let(:mail) { described_class.with(recipient: user, record: discussion, story: story).notify_new_discussion }
  ...
  describe "#notify_new_post" do
    ...
    let(:mail) { described_class.with(recipient: user, discussion: discussion, story: story, record: post).notify_new_post }
  ...
  describe "#invite_contributor" do
    let(:user) { create(:user) }
    let(:story) { create(:story, title: "Test Story") }
    let(:mail) { described_class.with(record: user, story: story).invite_contributor }
